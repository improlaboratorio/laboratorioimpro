...
function Card({ children, style }) {
  return <div className="app-card" style={{ background: "#fff", border: `1px solid ${T.line}`, borderRadius: 12, padding: 18, marginBottom: 14, ...style }}>{children}</div>;
}
...
function inputStyle(extra) {
  return { width: "100%", padding: "9px 11px", border: `1px solid ${T.line}`, borderRadius: 8, fontSize: 14, background: T.paper, color: T.ink, ...extra };
}
function btn(variant, extra) {
  const base = { display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 14px", borderRadius: 8, fontSize: 13.5, fontWeight: 700, border: "1px solid transparent" };
  if (variant === "primary") return { ...base, background: T.accent, color: "#fff", ...extra };
  if (variant === "ghost") return { ...base, background: "transparent", color: T.ink, border: `1px solid ${T.line}`, ...extra };
...
function SubTabs({ tabs, active, setActive }) {
  return (
    <div style={{ display: "flex", gap: 4, flexWrap: "wrap", borderBottom: `1px solid ${T.paperDim}`, marginBottom: 14 }}>
      {tabs.map((t) => (
        <button key={t.id} onClick={() => setActive(t.id)} style={{
          ...
          borderBottom: active === t.id ? `2px solid ${T.accent}` : "2px solid transparent", marginBottom: -1,
        }}>{t.label}</button>
      ))}
    </div>
  );
...
/* ============ DATOS POR DEFECTO ============ */
const EMPTY_ROSTER = { teachers: [], students: {}, dinamicasCatalog: [], tiposClase: [] };
...
function LoginGate({ roster, onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.paper, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "system-ui, sans-serif" }}>
...
export default function AsistenciaApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("asis_roster", true);
        ...
        setRoster(merged);
        if (!loaded.tiposClase || !loaded.dinamicasCatalog) {
          // guardamos ya el catálogo sembrado, para no depender de que ocurra otro guardado más tarde
          window.storage.set("asis_roster", JSON.stringify(merged), true).catch(() => {});
        }
      } catch (e) {}
      try {
        const s = await window.storage.get("asis_sessions", true);
        if (s && s.value) setSessions(JSON.parse(s.value));
      } catch (e) {}
      try {
        const p = await window.storage.get("asis_payments", true);
        if (p && p.value) setPayments({ ...EMPTY_PAYMENTS, ...JSON.parse(p.value) });
      } catch (e) {}
      try {
        const pd = await window.storage.get("asis_pending", true);
  ...
  const persistRoster = useCallback(async (next) => {
    setRoster(next);
    try {
      const res = await window.storage.set("asis_roster", JSON.stringify(next), true);
  ...
  const persistSessions = useCallback(async (next) => {
    setSessions(next);
    try {
      const res = await window.storage.set("asis_sessions", JSON.stringify(next), true);
      setError(res ? "" : "No se ha podido guardar. Inténtalo de nuevo.");
    } catch (e) { setError("No se ha podido guardar. Comprueba la conexión."); }
  }, []);
  const persistPayments = useCallback(async (next) => {
    setPayments(next);
    try {
      const res = await window.storage.set("asis_payments", JSON.stringify(next), true);
      setError(res ? "" : "No se ha podido guardar. Inténtalo de nuevo.");
    } catch (e) { setError("No se ha podido guardar. Comprueba la conexión."); }
  }, []);
  const persistPending = useCallback(async (next) => {
    setPending(next);
    try {
      const res = await window.storage.set("asis_pending", JSON.stringify(next), true);
      setError(res ? "" : "No se ha podido guardar. Inténtalo de nuevo.");
    } catch (e) { setError("No se ha podido guardar. Comprueba la conexión."); }
  }, []);
  ...
  const ctx = { roster, sessions, payments, pending, persistRoster, persistSessions, persistPayments, persistPending, addTeacher, updateTeacher, regenerateTeacherPassword, removeTeacher, addDinamica, removeDinamica, saveTipoClase, removeTipoClase, resetSessions, auth, isAdmin, askConfirm };

  return (
    <div style={{ minHeight: "100vh", background: T.paper, fontFamily: "system-ui, sans-serif", color: T.ink }}>
      <style>{`
      ...
      <header style={{ borderBottom: `3px solid ${T.ink}`, padding: "12px 12px 0" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
          <div style={{ minWidth: 0 }}>
            ...
            <h1 style={{ margin: "0 0 12px", fontSize: 24, fontWeight: 800 }}>Asistencia</h1>
          </div>
              ...
              {tabs.map((it) => {
                ...
                return (
                  <button key={it.id} onClick={() => setTab(it.id)} style={{
                    ...
                    border: "none", background: "transparent", fontSize: 13.5, fontWeight: active ? 700 : 500,
                    color: active ? T.ink : T.muted, borderBottom: active ? `3px solid ${T.accent}` : "3px solid transparent", marginBottom: -2,
                    whiteSpace: "nowrap",
...
function CalendarioTab({ ctx }) {
  const { roster, sessions, isAdmin, auth } = ctx;
  ...
  function handleDayClick(dateStr) {
    ...
    if (dayGroups.length === 0) {
      if (!isAdmin) { setToast("No tienes ninguna clase asignada este día."); setTimeout(() => setToast(""), 2200); }
      return;
    }
    if (dayGroups.length === 1) { setOpenSession({ date: dateStr, groupId: dayGroups[0].id }); return; }
  ...
  return (
    <div>
      <Card>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button style={btn("ghost", { padding: "4px 9px", fontSize: 11.5 })} onClick={() => setCursor(new Date(new Date().getFullYear(), new Date().getMonth(), 1))}>Hoy</button>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, marginBottom: 4 }}>
          {DOW_NAMES.map((d) => <div key={d} style={{ textAlign: "center", fontSize: 11, fontWeight: 700, color: T.muted, padding: "4px 0" }}>{d}</div>)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
          {cells.map((d, i) => {
            ...
            return (
              <button key={i} onClick={() => handleDayClick(dateStr)} style={{
                ...
                <div style={{ fontSize: 11, color: T.muted, padding: "4px 6px" }}>{d}</div>
                <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                  {sess.map((s) => {
                    ...
                    return (
                      <div key={s.id} title={`${s.profesor || "sin asignar"} — ${g?.label || ""}${given ? " (impartida)" : " (programada)"}`} style={{
                      ...
                      }}>{s.profesor ? s.profesor.trim()[0].toUpperCase() : "?"}</div>
                    );
                  })}
                </div>
              </button>
        ...
        <div style={{ display: "flex", gap: 14, marginTop: 12, fontSize: 11.5, color: T.muted, flexWrap: "wrap" }}>
      ...
      {pickerDate && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(10,10,10,0.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }} onClick={() => setPickerDate(null)}>
          <div style={{ background: "#fff", borderRadius: "16px 16px 0 0", width: "100%", maxWidth: 420, padding: 20 }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <div style={{ fontWeight: 800, fontSize: 15, textTransform: "capitalize" }}>{fmtDateLong(pickerDate)}</div>
              <button onClick={() => setPickerDate(null)} style={{ border: "none", background: "transparent", color: T.muted, display: "flex" }}><X size={18} /></button>
            </div>
              ...
              {dayGroupsFor(pickerDate).map((g) => {
                ...
                return (
                  <button key={g.id} onClick={() => { setOpenSession({ date: pickerDate, groupId: g.id }); setPickerDate(null); }} style={{
                    display: "flex", alignItems: "center", justifyContent: "space-between", padding: 13, borderRadius: 8,
...
function ClassEditor({ date, groupId, ctx, onClose }) {
  const { roster, sessions, persistSessions, pending, persistPending, addDinamica, isAdmin, auth, askConfirm } = ctx;
  ...
  function applyTipoClase(numeroStr) {
    ...
    const tipo = roster.tiposClase.find((t) => t.numero === numero);
    ...
    setDraft((d) => {
      ...
      return { ...d, tipoClase: numeroStr, dinamicas: [...tipoItems, ...extras], escenas: d.escenas || tipo.escenas || "" };
    });
  }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(10,10,10,0.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: "16px 16px 0 0", width: "100%", maxWidth: 620, maxHeight: "90vh", overflowY: "auto", padding: 20 }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: 16, textTransform: "capitalize" }}>{fmtDateLong(date)}</div>
            <div style={{ fontSize: 12.5, color: T.muted }}>{group.label}</div>
          </div>
          <button onClick={onClose} style={{ border: "none", background: "transparent", color: T.muted, display: "flex" }}><X size={20} /></button>
        </div>
        {existing && (
          <span style={{ fontSize: 10, fontWeight: 800, padding: "3px 8px", borderRadius: 999, background: given ? T.greenBg : T.accentDim, color: given ? T.green : T.accent, display: "inline-block", marginTop: 6, marginBottom: 8 }}>
            {given ? "IMPARTIDA" : "PROGRAMADA"}
          </span>
        )}
        ...
        {sub === "clase" && (
          <div>
            ...
            {students.length === 0 ? (
              <div style={{ fontSize: 13, color: T.muted }}>Este grupo no tiene alumnos activos todavía.</div>
            ) : (
              <div>
                ...
                {students.map((s) => {
                  ...
                  return (
                    <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", borderTop: `1px solid ${T.paperDim}` }}>
                      ...
                      <div style={{ fontSize: 14, color: present ? T.ink : T.muted }}>{s.name}</div>
                    </div>
                  );
                })}
              </div>
        ...
        {sub === "escenas" && (
          <div><Label>Escenas</Label><textarea style={inputStyle({ minHeight: 100, resize: "vertical" })} value={draft.escenas} onChange={(e) => patch({ escenas: e.target.value })} placeholder="Se rellena al cargar la plantilla del tipo de clase; puedes modificarla o dejarla tal cual." /></div>
        )}
        {sub === "titulos" && (
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 18, borderTop: `1px solid ${T.paperDim}`, paddingTop: 14 }}>
...
function GroupPicker({ groupId, setGroupId }) {
  const [day, setDay] = useState(GROUPS.find((g) => g.id === groupId)?.day || DAYS[0].key);
  const dayGroups = GROUPS.filter((g) => g.day === day);
  useEffect(() => {
    if (!dayGroups.some((g) => g.id === groupId)) setGroupId(dayGroups[0].id);
...
function AlumnosTab({ ctx }) {
  const { roster, persistRoster, sessions, persistSessions, payments } = ctx;
    ...
    Object.entries(roster.students).forEach(([gid, list]) => {
      list.forEach((s) => {
        if (s.active !== false && s.name.toLowerCase().includes(q)) {
          out.push({ student: s, group: GROUPS.find((g) => g.id === gid) });
        }
      });
    });
  ...
  const cols = useMemo(() => weekdayColumnsForRange(group.dow, 2), [group.dow]);
  ...
  return (
    <div>
      ...
      <Card>
        ...
        {searchResults.length > 0 && (
          <div style={{ marginTop: 8, borderTop: `1px solid ${T.paperDim}`, paddingTop: 8 }}>
            {searchResults.map((r) => (
              <button key={r.student.id} onClick={() => setOpenStudent(r)} style={{ display: "flex", justifyContent: "space-between", width: "100%", border: "none", background: "transparent", padding: "6px 4px", textAlign: "left", fontSize: 13.5 }}>
                <span>{r.student.name}</span><span style={{ color: T.muted, fontSize: 12 }}>{r.group?.label}</span>
              </button>
            ))}
          </div>
        )}
      ...
      <GroupPicker groupId={groupId} setGroupId={setGroupId} />

      <Card style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontSize: 13, color: T.muted }}>Mes actual y los dos anteriores · {group.label}</div>
...
function StudentModal({ student, group, ctx, onClose, onSaveDetails }) {
  const { sessions, payments, persistPayments, isAdmin } = ctx;
  ...
  const months = range === 1 ? [cursor] : [new Date(cursor.getFullYear(), cursor.getMonth() - 2, 1), new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1), cursor];
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(10,10,10,0.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 55 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: "16px 16px 0 0", width: "100%", maxWidth: 480, maxHeight: "88vh", overflowY: "auto", padding: 20 }} onClick={(e) => e.stopPropagation()}>
        ...
        {sub === "pagos" && (
          <div>
            {paymentMonths.map((ym) => {
              ...
              return (
                <div key={ym} style={{ border: `1px solid ${T.paperDim}`, borderRadius: 8, padding: 10, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, textTransform: "capitalize" }}>{monthLabel(ym)}</div>
                    {isAdmin ? (
                      <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
                        <input type="checkbox" checked={!!p.paid} onChange={(e) => setPayment(ym, { paid: e.target.checked })} /> Pagado
                      </label>
                    ) : (
                      <span style={{ fontSize: 11, fontWeight: 800, color: p.paid ? T.green : T.muted }}>{p.paid ? "PAGADO" : "PENDIENTE"}</span>
                    )}
                  </div>
                  {isAdmin && p.paid && (
                    <div className="two-col" style={{ gap: 8 }}>
                      <input type="number" style={inputStyle({ padding: "6px 9px", fontSize: 12.5 })} placeholder="Importe (€)" value={p.amount || ""} onChange={(e) => setPayment(ym, { amount: e.target.value })} />
                      <input style={inputStyle({ padding: "6px 9px", fontSize: 12.5 })} placeholder="A quién" value={p.paidTo || ""} onChange={(e) => setPayment(ym, { paidTo: e.target.value })} />
                    </div>
                  )}
    ...
    <div>
      <Card>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
          {roster.teachers.map((t) => (
            <button key={t.name} onClick={() => setOpenTeacherName(t.name)} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: T.paperDim, borderRadius: 999, padding: "6px 7px 6px 12px", fontSize: 13.5, border: "none" }}>
...
function DinamicasTab({ ctx }) {
  ...
  function startEditTipo(tipo) {
    ...
    setTipoDraft(tipo ? { ...tipo, dinamicaIds: [...tipo.dinamicaIds] } : { numero: "", nombre: "", dinamicaIds: [], escenas: "" });
  }
  ...
  function saveTipoDraft() {
    ...
    saveTipoClase({ numero, nombre: "", dinamicaIds: tipoDraft.dinamicaIds, escenas: tipoDraft.escenas });
        ...
        <div>
          ...
          {roster.tiposClase.slice().sort((a, b) => a.numero - b.numero).map((tipo) => (
            <Card key={tipo.numero}>
              <div style={{ fontWeight: 800, fontSize: 14.5, marginBottom: 8 }}>Tipo {tipo.numero}</div>
        ...
        <div>
          ...
          {!editingTipo && roster.tiposClase.slice().sort((a, b) => a.numero - b.numero).map((tipo) => (
            <Card key={tipo.numero} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 14.5 }}>Tipo {tipo.numero}</div>
