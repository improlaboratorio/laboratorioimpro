...
function PagosTab({ ctx }) {
  ...
  const activeGroupId = modo === "grupos" ? groupId : sporadicId;
  ...
  // Más reciente primero (a la izquierda) — igual que en Asistencia — para ver de un
  // vistazo el mes actual sin tener que deslizar; los meses anteriores quedan a la derecha.
  const months = useMemo(() => {
    const now = new Date();
    const arr = [];
    for (let i = 0; i <= 11; i++) arr.push(yyyymm(new Date(now.getFullYear(), now.getMonth() - i, 1)));
    return arr;
  }, []);
  ...
  // Total de dinero de TODAS las clases (grupos fijos + sueltas) para el mes elegido.
  ...
  const totalGeneral = useMemo(() => {
    ...
    todosLosGrupos.forEach((g) => {
      (roster.students[g.id] || []).filter((s) => s.active !== false).forEach((s) => {
        const p = (payments.students[s.id] || {})[ym] || {};
        if (p.paid) sum += Number(p.amount || 0);
      });
    });
    return sum;
  }, [roster.students, payments.students, ym, todosLosGrupos.length]);

  // Meses en los que TODOS los alumnos activos de la clase elegida ya han pagado (para
  // pintar ese mes en verde en la tira de meses).
  const mesesCompletos = useMemo(() => {
    if (!group || students.length === 0) return new Set();
    const set = new Set();
    months.forEach((m) => {
      const completo = students.every((s) => (payments.students[s.id] || {})[m]?.paid);
      if (completo) set.add(m);
    });
    return set;
        ...
        <>
          <Card style={{ padding: "10px 0" }}>
            <div className="tab-scroll" style={{ padding: "0 14px", gap: 6 }}>
              {months.map((m) => {
                const selected = ym === m;
                const completo = mesesCompletos.has(m);
                return (
                  <button key={m} onClick={() => setYm(m)} style={{
                    ...
                    color: selected ? "#fff" : completo ? T.green : T.muted, flexShrink: 0,
                  }}>{monthLabel(m)}{completo && !selected ? " ✓" : ""}</button>
                );
              })}
            </div>
          </Card>
          ...
          <div className="two-col" style={{ gap: 14 }}>
            ...
            <Card style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <Label>Total de todas las clases — {monthLabel(ym)}</Label>
              <div style={{ fontSize: 22, fontWeight: 800 }}>{totalGeneral} €</div>
            </Card>
          </div>

          {students.length > 0 && (
            <Card style={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
              {pendientes.length === 0
