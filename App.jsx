...
import {
  ...
  LogOut, Sparkles, UserX, Eye, Euro
} from "lucide-react";
...
function Card({ children, style }) {
  return <div className="app-card" style={{ background: "#fff", border: `1px solid ${T.line}`, borderRadius: 16, padding: 20, marginBottom: 16, boxShadow: "0 1px 2px rgba(10,10,10,0.04), 0 6px 16px -8px rgba(10,10,10,0.08)", ...style }}>{children}</div>;
}
...
function btn(variant, extra) {
  ...
  if (variant === "primary") return { ...base, background: T.accent, color: "#fff", boxShadow: "0 4px 10px -4px rgba(227,20,20,0.55)", ...extra };
              ...
              {tabs.map((it) => {
                ...
                return (
                  <button key={it.id} onClick={() => setTab(it.id)} className="app-tab" style={{
                    ...
                    border: "none", background: active ? T.ink : "transparent", fontSize: 13.5, fontWeight: active ? 700 : 500,
...
function CalendarioTab({ ctx }) {
  const { roster, sessions, isAdmin, auth, persistRoster } = ctx;
  ...
  const [sporadicTipo, setSporadicTipo] = useState("individual");
  ...
  function createSporadicClass() {
    const name = sporadicName.trim();
    if (!name) return;
    const newGroup = { id: `suelta-${uid()}`, label: name, level: name, tipo: sporadicTipo, day: null, dow: null };
    persistRoster({ ...roster, clasesSueltas: [...(roster.clasesSueltas || []), newGroup], students: { ...roster.students, [newGroup.id]: [] } });
    setOpenSession({ date: pickerDate, groupId: newGroup.id });
    setPickerDate(null);
    setCreatingSporadic(false);
    setSporadicName("");
    setSporadicTipo("individual");
  }
  ...
  return (
    <div>
      ...
      {pickerDate && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(10,10,10,0.45)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }} onClick={() => { setPickerDate(null); setCreatingSporadic(false); }}>
          <div className="modal-sheet-pad" style={{ background: "#fff", borderRadius: "16px 16px 0 0", width: "100%", maxWidth: 420, padding: 20 }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              ...
              <button onClick={() => { setPickerDate(null); setCreatingSporadic(false); }} style={{ border: "none", background: "transparent", color: T.muted, display: "flex" }}><X size={18} /></button>
            </div>
              ...
              {(byDate[pickerDate] || []).filter((s) => !GROUPS.some((g) => g.id === s.groupId)).map((s) => {
                ...
                return (
                  <button key={s.id} onClick={() => { setOpenSession({ date: pickerDate, groupId: g.id }); setPickerDate(null); }} style={{
                    ...
                    <span style={{ fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>{g.level} <Badge tone="neutral">{g.tipo === "individual" ? "Individual" : "Grupal"}</Badge></span>
            ...
            {isAdmin && creatingSporadic && (
              <div style={{ marginTop: 12, borderTop: `1px solid ${T.paperDim}`, paddingTop: 12 }}>
                ...
                <input autoFocus style={inputStyle({ marginBottom: 10 })} placeholder="Ej. Voz individual, Taller de clown…" value={sporadicName} onChange={(e) => setSporadicName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && createSporadicClass()} />
                <Label>Tipo</Label>
                <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
                  <button style={btn(sporadicTipo === "individual" ? "primary" : "ghost", { flex: 1, justifyContent: "center", fontSize: 12.5 })} onClick={() => setSporadicTipo("individual")}>Individual</button>
                  <button style={btn(sporadicTipo === "grupal" ? "primary" : "ghost", { flex: 1, justifyContent: "center", fontSize: 12.5 })} onClick={() => setSporadicTipo("grupal")}>Grupal</button>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button style={btn("ghost", { flex: 1, justifyContent: "center" })} onClick={() => { setCreatingSporadic(false); setSporadicName(""); }}>Cancelar</button>
...
function ClassEditor({ date, groupId, ctx, onClose }) {
  const { roster, sessions, persistSessions, persistRoster, pending, persistPending, addDinamica, isAdmin, auth, askConfirm } = ctx;
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
          <div>
            ...
            <div style={{ fontSize: 12.5, color: T.muted, display: "flex", alignItems: "center", gap: 6 }}>
              {group.label}
              {isSporadic && <Badge tone="accent">{group.tipo === "individual" ? "Clase individual" : "Clase grupal"}</Badge>}
            </div>
          </div>
          <button onClick={onClose} style={{ border: "none", background: "transparent", color: T.muted, display: "flex" }}><X size={20} /></button>
        </div>
...
function PagosTab({ ctx }) {
  ...
  const [modo, setModo] = useState("grupos"); // grupos | sueltas
  const [dayKey, setDayKey] = useState(DAYS[0].key); // uno de DAYS, o "total"
  const [groupId, setGroupId] = useState(GROUPS.find((g) => g.day === DAYS[0].key).id);
  const [sporadicId, setSporadicId] = useState(null);
  // "windowEnd" es el mes más reciente/futuro visible en la tira (el de más a la
  // izquierda); se puede mover libremente hacia atrás o hacia adelante, años incluidos,
  // para poder registrar pagos por adelantado.
  const [windowEnd, setWindowEnd] = useState(() => { const d = new Date(); return new Date(d.getFullYear(), d.getMonth(), 1); });
  const [ym, setYm] = useState(yyyymm(new Date()));

  const clasesSueltas = roster.clasesSueltas || [];
  const isTotal = modo === "grupos" && dayKey === "total";
  const activeGroupId = modo === "grupos" ? (isTotal ? null : groupId) : sporadicId;
  ...
  // Mes más reciente/futuro a la izquierda, retrocediendo hacia la derecha — igual que
  // en Asistencia — pero ahora la ventana de 12 meses se puede desplazar con las flechas.
  const months = useMemo(() => {
    const arr = [];
    for (let i = 0; i < 12; i++) arr.push(yyyymm(new Date(windowEnd.getFullYear(), windowEnd.getMonth() - i, 1)));
    return arr;
  }, [windowEnd]);

  function shiftWindow(deltaMonths) {
    setWindowEnd((d) => new Date(d.getFullYear(), d.getMonth() + deltaMonths, 1));
  }
  function goToday() {
    const d = new Date();
    setWindowEnd(new Date(d.getFullYear(), d.getMonth(), 1));
    setYm(yyyymm(d));
  }
  ...
  const totalCobrado = rows.reduce((sum, r) => sum + (r.p.paid ? Number(r.p.amount || 0) : 0), 0);
  ...
  return (
    <div>
      <Card>
        <SubTabs tabs={[{ id: "grupos", label: "Grupos fijos" }, { id: "sueltas", label: "Clases sueltas" }]} active={modo} setActive={setModo} />
        {modo === "grupos" ? (
          <div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
              {DAYS.map((d) => (
                <button key={d.key} onClick={() => { setDayKey(d.key); const g = GROUPS.find((x) => x.day === d.key); if (g) setGroupId(g.id); }} style={{
                  padding: "7px 13px", borderRadius: 8, fontSize: 13, fontWeight: 600,
                  border: `1px solid ${dayKey === d.key ? T.ink : T.line}`,
                  background: dayKey === d.key ? T.ink : "#fff", color: dayKey === d.key ? "#fff" : T.ink,
                }}>{d.label}</button>
              ))}
              <button onClick={() => setDayKey("total")} style={{
            ...
            {!isTotal && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {GROUPS.filter((g) => g.day === dayKey).map((g) => (
                  <button key={g.id} onClick={() => setGroupId(g.id)} style={{
                    padding: "7px 13px", borderRadius: 8, fontSize: 13, fontWeight: 600,
          ...
          <div style={{ fontSize: 13, color: T.muted }}>Todavía no has creado ninguna clase suelta. Se crean desde el Calendario, al clicar un día → "Crear clase nueva".</div>
        ) : (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {clasesSueltas.map((g) => (
      ...
      <Card style={{ padding: "10px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 14px 8px" }}>
          <button style={btn("ghost", { padding: "5px 9px" })} onClick={() => shiftWindow(-12)}><ChevronLeft size={14} /> Antes</button>
          <button style={btn("ghost", { padding: "5px 10px", fontSize: 11.5 })} onClick={goToday}>Hoy</button>
          <button style={btn("ghost", { padding: "5px 9px" })} onClick={() => shiftWindow(12)}>Después <ChevronRight size={14} /></button>
        </div>
        <div className="tab-scroll" style={{ padding: "0 14px", gap: 6 }}>
          {months.map((m) => (
            <button key={m} onClick={() => setYm(m)} style={{
              ...
              background: ym === m ? T.ink : T.paperDim, color: ym === m ? "#fff" : T.muted, flexShrink: 0,
            }}>{monthLabel(m)}</button>
          ))}
        </div>
      </Card>
      ...
      {isTotal ? (
            ...
            {todosLosGrupos.map((g) => {
              ...
              return (
                <div key={g.id} style={{
                  ...
                  {g.label}
                  {sts.length === 0 && <div style={{ fontSize: 10.5, fontWeight: 500, opacity: 0.75, marginTop: 2 }}>Sin alumnos</div>}
                </div>
              );
            })}
          </div>
        ...
        <>
          <Card style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            <div>
              <Label>Cobrado en {group.label} — {monthLabel(ym)}</Label>
              <div style={{ fontSize: 22, fontWeight: 800 }}>{totalCobrado} €</div>
            </div>
            {students.length > 0 && (
              pendientes.length === 0
        ...
        <div>
          ...
          {roster.tiposClase.slice().sort((a, b) => a.numero - b.numero).map((tipo) => (
            <Card key={tipo.numero}>
              <div style={{ fontWeight: 800, fontSize: 14.5, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>Tipo {tipo.numero} <Badge tone="accent">{tipo.nivel || "Iniciación 1"}</Badge></div>
        ...
        <div>
          ...
          {!editingTipo && roster.tiposClase.slice().sort((a, b) => a.numero - b.numero).map((tipo) => (
            <Card key={tipo.numero} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 14.5, display: "flex", alignItems: "center", gap: 6 }}>Tipo {tipo.numero} <Badge tone="accent">{tipo.nivel || "Iniciación 1"}</Badge></div>
