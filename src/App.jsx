...
export default function AsistenciaApp() {
  ...
  const ctx = { roster, sessions, payments, pending, persistRoster, persistSessions, persistPayments, persistPending, addTeacher, updateTeacher, regenerateTeacherPassword, removeTeacher, addDinamica, removeDinamica, saveTipoClase, removeTipoClase, resetSessions, auth, isAdmin, askConfirm, focusGroupId, setFocusGroupId, goToAlumnosGroup };
...
function CalendarioTab({ ctx }) {
  const { roster, sessions, isAdmin, auth, persistRoster, persistSessions, addDinamica, goToAlumnosGroup } = ctx;
  ...
  const [sporadicDraft, setSporadicDraft] = useState(() => ({ nombre: "", profesor: "", dinamicas: [], escenas: "", titulos: "", conceptosFinales: "", observaciones: "" }));
  ...
  function resetSporadicDraft() {
    setSporadicDraft({ nombre: "", profesor: "", dinamicas: [], escenas: "", titulos: "", conceptosFinales: "", observaciones: "" });
    setNewDinSporadic("");
  }
  ...
  function createSporadicClass() {
    ...
    const newSession = {
      ...
      estado: pickerDate <= iso(new Date()) ? "impartida" : "programada",
          ...
          <div className="modal-sheet-pad" style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 420, maxHeight: "85vh", overflowY: "auto", padding: 20 }} onClick={(e) => e.stopPropagation()}>
            ...
            {isAdmin && creatingSporadic && (
              <div style={{ marginTop: 12, borderTop: `1px solid ${T.paperDim}`, paddingTop: 12 }}>
                ...
                <Label>Escenas</Label>
                <textarea style={inputStyle({ minHeight: 70, resize: "vertical", marginBottom: 10 })} value={sporadicDraft.escenas} onChange={(e) => setSporadicDraft((d) => ({ ...d, escenas: e.target.value }))} />
                <Label>Títulos</Label>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
          <div>
            ...
            <div style={{ fontSize: 12.5, color: T.muted, display: "flex", alignItems: "center", gap: 6 }}>
              {group.label}
              {isSporadic && <Badge tone="accent">Clase suelta</Badge>}
            </div>
          </div>
          <button onClick={onClose} style={{ border: "none", background: "transparent", color: T.muted, display: "flex" }}><X size={20} /></button>
        </div>
